...
def responders_vs_nonresponders_stats(conn, freq_df):
    ...
    if significant.empty:
        print(
            "No statistically significant differences found (p >= 0.05 for all populations)."
        )
    else:
        print(significant.to_string(index=False))

    return {"stats": stats_df, "data": merged}
...
def plot_boxplot(analysis):
    ...
    # Save plot
    plt.savefig(
        os.path.join(OUTPUT_DIR, "pt3_responder_yn_boxplot.png"), bbox_inches="tight"
    )
